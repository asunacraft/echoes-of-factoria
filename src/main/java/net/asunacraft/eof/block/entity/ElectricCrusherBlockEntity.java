...
/**
 * Tier-1 machine: crushes ores and cracked ores into metal dust.
 * LV, 1 amp (32 FE/t), 2400 FE per operation.
 */
public class ElectricCrusherBlockEntity extends AbstractElectricMachineBlockEntity implements MenuProvider, ContainerData {
    private static final int SLOT_INPUT = 0;
    private static final int SLOT_OUTPUT = 1;
    private static final int TOTAL_ENERGY = 2400;
    ...
    public ElectricCrusherBlockEntity(BlockPos pos, BlockState state) {
        super(EOFBlockEntities.ELECTRIC_CRUSHER.get(), pos, state, VoltageTier.LV, 1, 4096);
    }
    ...
    private Optional<CrushingRecipe> findRecipe(ItemStack input) {
        ...
        return level.getRecipeManager().getRecipeFor(EOFRecipeTypes.CRUSHING.get(), containerOf(input), level);
    }
    ...
    protected boolean canCraft() {
        ...
        Optional<CrushingRecipe> recipe = findRecipe(input);
        if (recipe.isEmpty()) {
            return false;
        }
    ...
    protected void onTickServer() {
        ...
        if (level.getBlockState(worldPosition).getValue(ElectricCrusherBlock.LIT) != lit) {
            level.setBlock(worldPosition, level.getBlockState(worldPosition).setValue(ElectricCrusherBlock.LIT, lit), 3);
        }
    }
    ...
    public Component getDisplayName() {
        return Component.translatable("container.eof.electric_crusher");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new ElectricCrusherMenu(id, inv, this);
    }
